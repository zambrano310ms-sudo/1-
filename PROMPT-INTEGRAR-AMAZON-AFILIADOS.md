# Prompt para integrar la skill amazon-afiliados en otra cuenta de Claude Code

Copia TODO este archivo y pégalo como un único mensaje en una sesión de Claude Code abierta en el proyecto donde quieras tener la skill. Claude creará la estructura `.claude/skills/amazon-afiliados/` con sus 3 archivos.

Esta es una skill **orquestadora** que monta un negocio completo de afiliación de Amazon de principio a fin, encadenando las otras 10 skills del kit (prospeccion, web-scrolling, auditoria-seo, instagram-a-web, automatizaciones-n8n, auditoria-meta-ads, dashboard-facturas). Si no las tienes instaladas, instala primero las 10 con el otro prompt (`PROMPT-INTEGRAR-10-SKILLS.md`).

---

INSTRUCCIONES PARA CLAUDE:

Crea en la raíz del proyecto actual la carpeta `.claude/skills/amazon-afiliados/` y dentro 3 archivos con el contenido exacto que viene entre los marcadores `===BEGIN <archivo>===` y `===END <archivo>===` (sin incluir los marcadores).

Importante:
- El frontmatter YAML al principio del SKILL.md (entre `---` y `---`) DEBE preservarse exactamente.
- No interpretes ni resumas el contenido: cópialo VERBATIM, incluidos los bloques de código, emojis, tablas ASCII y saltos de línea.
- Si la carpeta `.claude/skills/` ya existe, NO la borres; solo añade lo que falte.
- Si la subcarpeta `amazon-afiliados/` ya existe, sobreescribe los 3 archivos.
...
└── skills/
    └── amazon-afiliados/
        ...
        ├── CLAUDE.md         ← mensaje de bienvenida
        └── INSTRUCCIONES.md  ← guía paso a paso
```

Al terminar, confírmame los 3 archivos creados.

---

## Los 3 archivos del kit amazon-afiliados


### SKILL.md — La skill (la reconoce Claude Code)
...
### CLAUDE.md — Mensaje de bienvenida del kit
...
### INSTRUCCIONES.md — Guía de uso del kit

===BEGIN INSTRUCCIONES.md===
...
===END INSTRUCCIONES.md===


---

Cuando termines, dime:
1. Qué carpeta y archivos se han creado.
2. Si ya existía algo y qué has sobrescrito.

Una vez instalada, abre la carpeta del proyecto en Claude Code y escribe algo como "montamos el negocio de afiliados de Amazon" y la skill se activará.
